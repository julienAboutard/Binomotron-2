...
# mycursor.execute("CREATE TABLE IF NOT EXISTS Work (idStudents INT REFERENCES Students(id), idBriefs INT REFERENCES Briefs(id), Group_name VARCHAR(255), PRIMARY KEY(idStudents,idBriefs))")

# Making sure the table is empty
mycursor.execute("TRUNCATE TABLE Students")
mycursor.execute("TRUNCATE TABLE Briefs") 
# mycursor.execute("TRUNCATE TABLE Work")
